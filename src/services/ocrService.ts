...
async function locateLotZone(uri: string): Promise<string | null> {
  try {
    ...
    for (const block of blocks) {
      ...
      // Seuil : un simple nombre 6-12 chiffres seul (+30) = prix / code produit /
      // fragment de code-barres, PAS un lot fiable (cas réels : "2103000" prix,
      // bandes 417x850 / 264x1054 de charabia). On exige au moins un signal dense
      // lettre+chiffre (+40), un L-code (+80) ou le mot LOT (+100) pour déclencher
      // une zone — sinon on laisse la bande centrale pleine largeur, qui marche.

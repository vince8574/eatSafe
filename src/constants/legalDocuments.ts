// Import markdown files as strings
export const PRIVACY_POLICY = `# Politique de Confidentialité - Eats OK

*Dernière mise à jour : 12 décembre 2025*

**Eats OK** s'engage à protéger la vie privée et les données personnelles de ses utilisateurs. La présente Politique de Confidentialité vous informe sur la manière dont nous collectons, utilisons, stockons et protégeons vos données personnelles conformément au Règlement Général sur la Protection des Données (RGPD) et à la loi Informatique et Libertés.

---

## 1. Responsable du traitement des données

**Identité du responsable de traitement :**
- Nom : [Votre Nom/Société]
- Adresse : [Votre adresse complète]
- Email : contact@eatsok.app
- Numéro SIRET : [Votre numéro SIRET]

**Délégué à la Protection des Données (DPO) :**
- Email : dpo@eatsok.app
- Contact : privacy@eatsok.app

## 2. Données collectées

### 2.1 Données que nous NE collectons PAS

❌ **Nous ne collectons PAS :**
- Vos photos de produits (supprimées immédiatement après analyse)
- Votre historique de navigation
- Votre géolocalisation
- Vos contacts
- Vos données bancaires

### 2.2 Données collectées localement

Ces données sont stockées **uniquement sur votre smartphone** :
- Prénom (optionnel)
- Langue choisie
- Historique des scans
- Préférences utilisateur

## 3. Vos droits (RGPD)

Conformément au RGPD, vous disposez des droits suivants :
- Droit d'accès
- Droit de rectification
- Droit à l'effacement
- Droit à la portabilité
- Droit d'opposition

**Contact** : privacy@eatsok.app

---

**Votre vie privée est notre priorité. Merci de faire confiance à Eats OK.**
`;

export const TERMS_OF_SERVICE = `# Conditions Générales d'Utilisation - Eats OK

*Dernière mise à jour : 12 décembre 2025*

## 1. Objet

Les présentes Conditions Générales d'Utilisation (CGU) régissent l'accès et l'utilisation de l'application mobile **Eats OK**.

## 2. Description du service

Eats OK est une application mobile permettant de :
- Scanner les produits alimentaires
- Vérifier leur statut de rappel
- Recevoir des alertes en cas de rappel
- Consulter l'historique des scans

## 3. Technologie OCR

L'application utilise la technologie OCR (Reconnaissance Optique de Caractères) via Google ML Kit pour extraire le texte des photos de produits.

**Important** : La reconnaissance OCR n'est pas infaillible et peut comporter des erreurs.

## 4. Responsabilités de l'utilisateur

L'utilisateur s'engage à :
- Utiliser l'application de manière conforme à sa destination
- Ne pas détourner l'application de son usage normal
- Vérifier les informations obtenues

## 5. Limitation de responsabilité

Eats OK ne saurait être tenu responsable :
- Des erreurs de reconnaissance OCR
- Des décisions prises sur la base des informations fournies
- De l'exhaustivité des données de rappel

No warranty, express or implied, is provided regarding the accuracy or completeness of the information.

## 6. Contact

Pour toute question : contact@eatsok.app

---

**En utilisant Eats OK, vous acceptez les présentes CGU.**
`;

export const LEGAL_NOTICE = `# Mentions Légales - Eats OK

*Dernière mise à jour : 12 décembre 2025*

## 1. Éditeur de l'application

**Identité de l'éditeur :**
- Nom : [Votre Nom/Société]
- Forme juridique : [Forme juridique]
- Adresse : [Votre adresse complète]
- Email : contact@eatsok.app
- Numéro SIRET : [Votre numéro SIRET]

## 2. Hébergement

**Hébergeur de l'application :**
- Nom : Google LLC (Firebase)
- Adresse : 1600 Amphitheatre Parkway, Mountain View, CA 94043, États-Unis
- Site web : https://firebase.google.com

## 3. Propriété intellectuelle

L'application Eats OK, son design, son logo et tous les éléments qui la composent sont protégés par le droit d'auteur et la propriété intellectuelle.

**Toute reproduction, même partielle, est interdite sans autorisation.**

## 4. Protection des données personnelles

Conformément au RGPD, vous disposez de droits sur vos données personnelles.

**Délégué à la Protection des Données (DPO) :**
- Email : dpo@eatsok.app
- Contact : privacy@eatsok.app

## 5. Contact

Pour toute question relative aux mentions légales :
- Email : contact@eatsok.app

---

**Merci d'utiliser Eats OK.**
`;

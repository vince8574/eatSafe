...
type DietaryProfileState = {
  ...
  // Consentement EXPLICITE au traitement des données de santé, horodaté + version
  // de la politique acceptée. Requis AVANT toute saisie. Synchronisé côté serveur
  // (getProfile) pour l'accountability. Retiré en supprimant ses profils.
    ...
    (set, get) => {
      ...
      return {
        ...
        addCustomAvoidFood: (personId, food) =>
          updatePerson(personId, (p) => {
            const v = food.trim();
            // dédoublonnage insensible à la casse ; ignore le vide.
